...
export default function AiMlHero() {
  ...
  useGSAP(
    () => {
      ...
      const split = new SplitText(".aiml-heading", { type: "lines" });
      const tl = gsap.timeline();

      tl.from(".aiml-badge", { y: 12, opacity: 0 })
        .from(split.lines, { y: 24, opacity: 0, stagger: 0.08 }, "-=0.3")
        .from(".aiml-copy", { y: 16, opacity: 0 }, "-=0.4")
        .from(".aiml-cta > *", { y: 16, opacity: 0, stagger: 0.08 }, "-=0.4");
  ...
  return (
    <div ref={rootRef} className="relative overflow-hidden bg-background">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col items-center px-6 pt-16 pb-24 text-center sm:pt-20">
        <div className="aiml-badge mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-surface px-4 py-1.5 text-sm text-foreground-muted">
          <span className="h-1.5 w-1.5 rounded-full bg-primary" />
          AI/ML Solutions
        </div>

        <h1 className="aiml-heading max-w-3xl text-4xl font-semibold leading-[1.1] tracking-tight text-foreground sm:text-5xl md:text-6xl">
          Models that solve a real problem, not a demo.
        </h1>

        <p className="aiml-copy mt-6 max-w-md text-lg leading-8 text-foreground-muted">
          From a first prototype to a production model that holds up under real traffic and
          real data.
        </p>

        <div className="aiml-cta mt-10 flex flex-col gap-4 text-base font-medium sm:flex-row">
          <StartProjectButton />
          <a
            href="#offerings"
